...
export default function RightPanel({ show, onClose }) {
  const [page, setPage] = useState(null) // null=menu, 'diary','schedule','memory'
        ...
        {!page ? (
          <>
            ...
            <div className="rp-menu">
              <button className="lp-menu-item" onClick={() => setPage('diary')}>
                ...
                <span>日记</span>
                <span className="rp-badge">{diary.length}</span>
              </button>
              <button className="lp-menu-item" onClick={() => setPage('schedule')}>
                ...
                <span>行程</span>
                <span className="rp-badge">{schedule.length}</span>
              </button>
              <button className="lp-menu-item" onClick={() => setPage('memory')}>
                ...
                <span>记忆</span>
                <span className="rp-badge">{memory.length}</span>
              </button>
            </div>
          </>
        ) : (
          <>
            <div className="rp-page-hd">
              <button className="rp-back" onClick={goBack}>
            ...
            <div className="rp-page-body">
              <div className="rp-irow">
                <input value={input} onChange={e => setInput(e.target.value)} placeholder={page==='diary'?'记录今天...':page==='schedule'?'添加行程...':'记录一个记忆...'}
              ...
              <div className="elist">
                {data.map(e => (
                  <div key={e.id} className="eitem">
                    <div className="eitem-top"><span className="edate">{e.date}</span><button className="edel" onClick={() => del(e.id)}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M18 6L6 18M6 6l12 12"/></svg></button></div>
                    <p>{e.text}</p>
                  </div>
                ))}
